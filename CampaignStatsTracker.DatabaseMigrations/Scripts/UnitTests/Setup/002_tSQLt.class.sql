...
BEGIN
  ...
  SET @Cmd =
     'CREATE TRIGGER ____SCHEMA_NAME____.____TRIGGER_NAME____
      ON ____SCHEMA_NAME____.____VIEW_NAME____ INSTEAD OF INSERT AS
      BEGIN
  ...
  SET @Cmd = REPLACE(@Cmd, '____SCHEMA_NAME____', QUOTENAME(@SchemaName));
  SET @Cmd = REPLACE(@Cmd, '____VIEW_NAME____', QUOTENAME(@ViewName));
  SET @Cmd = REPLACE(@Cmd, '____TRIGGER_NAME____', QUOTENAME(@TriggerName));
  EXEC(@Cmd);
